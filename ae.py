...
def recommedations(input):
    ...
    if torch.sum(input.data > 0) > 0:
        ...
        ratings_series = pd.Series(output).sort_values(ascending = False)
        top_10_indices = list(ratings_series.iloc[0:10].index)
        
        for i in top_10_indices:
            recommended_movies.append(movies[1][i+1])
        
    else:
        print ('No rating history of the user')
        
    return recommended_movies    
    
rm = recommedations(Variable(test_set[0]).unsqueeze(0))
